...
function ENT:CustomOnInitialize()
	...
	self.ExtraGunModel:SetModel("models/gredwitch/mg42/mg42_gun_fix.mdl")
	self.ExtraGunModel:SetLocalPos(self:GetPos() + self:GetUp()*43)
...
function ENT:CustomOnThink()
	if IsValid(self.ExtraGunModel) then
		...
		if self.Emp_CurrentAmmo <= 0 && self.Emp_Reloading == false then
			...
			VJ_EmitSound(self, "vj_emplacements/mg42/mg42_reload.wav")
			...
			timer.Simple(0.6,function()
				if IsValid(self) then
					...
					self.ReloadAmmoBox:SetModel("models/gredwitch/mg42/mg42_belt.mdl")
...
function ENT:CustomRangeAttackCode()
	...
	self:PlaySoundSystem("RangeAttack", "vj_emplacements/mg42/mg42_tp.wav", VJ_EmitSound)
	VJ_EmitSound(self, "vj_emplacements/mg42/mg42_dist.wav", 110)
	...
	bulletShell:SetOrigin(self.ExtraGunModel:GetPos() + self:GetForward()*-8)
